...
| R16 | [Deciding which prose rules to adopt, and the interface for doing it](R16-prose-rule-decision-interfaces.md) | A second failed attempt at S06's rule-set question — the AI-written corpus was again treated as a voice to protect — plus `write-good`/Google measurements over 17,657 words and em-dash/semicolon densities. Surveys prior art for prose-rule *adoption* (almost none exists), borrows interaction design from annotation, RLHF, and static-analysis triage tooling, records the measured throughput and fatigue numbers, and frames three mechanisms: eliciting preferences against samples, mining the author's own edits, and calibrating rules against a known-good corpus. |
