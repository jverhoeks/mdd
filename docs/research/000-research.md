...
| R16 | [Deciding which prose rules to adopt, and the interface for doing it](R16-prose-rule-decision-interfaces.md) | A second failed attempt at S06's rule-set question, and why it failed the same way R15 did — the AI-written corpus was encoded as a voice to protect. Measurements for `write-good` plus the Google package over 17,657 words; em-dash and semicolon densities. A survey finding that nobody has built a prose-rule *adoption* interface: the Vale ecosystem, LanguageTool's two unjoined halves, Acrolinx's legacy Guidance Wizard, and the deleted ESLint autoconfig. Two mechanisms — eliciting preferences against samples, versus mining the author's own edits with git provenance — plus corpus calibration against known-good writing. Borrows interaction design from annotation and RLHF tooling (Prodigy, Argilla, Label Studio, OpenAI's anchored preference slider) and from static-analysis triage (CodeQL Quick Evaluation, Semgrep turbo mode, GitHub's dismissal data model, Coverity's triage store), and records what is actually measured about judging throughput, soft deadlines, criterion drift under class imbalance, and fatigue. |
